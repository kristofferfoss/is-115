...
// For extra safety against SQL-injections, can sort by following columns:
$columns = array('usersUid','usersRegdate','usersLastname','userInterests');

// Collects column if in the array above
$column = isset($_GET['column']) && in_array($_GET['column'], $columns) ? $_GET['column'] : $columns[0];

//Gets sorting method, ascending and descending, asc default
$sort_order = isset($_GET['order']) && strtolower($_GET['order']) == 'desc' ? 'DESC' : 'ASC';

// Get the result...
if ($result = $mysqli->query('SELECT * FROM users ORDER BY ' .  $column . ' ' . $sort_order)) 
{
	//Variables we need for the table
